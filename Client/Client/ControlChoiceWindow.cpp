...
int ControlChoiceWindow::GetKeyEventFromChar(char letter)
{
    ...
    return keyMap.value(letter, -1);  // Return -1 if the letter is not found
}
...
void ControlChoiceWindow::OnSaveControls()
{
    // Use the KeyHelper to map the control text to its corresponding integer key value
    ...
    // Debug: Print the controls
    qDebug() << "Controls collected:";
    for (auto it = controls.begin(); it != controls.end(); ++it) {
        qDebug() << it.key() << ": " << it.value();
    }

    // Emit the signal to save the controls
    //emit ControlsSet(controls, m_username);
